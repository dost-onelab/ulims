<?php
	...
	* @version 0.3
	*/	
	class EExcelViewCashReceiptsRecord extends CGridView
	{
		//Document properties
		...
		public function renderHeader()
		{
			...
			$this->objPHPExcel->getActiveSheet()->getColumnDimension('G')->setWidth(14);
			...
			$this->objPHPExcel->getActiveSheet()->getStyle('A1:H3')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			$this->objPHPExcel->getActiveSheet()->getStyle('E4:H4')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			...
			$this->objPHPExcel->getActiveSheet()->SetCellValue('A1', 'CASH RECEIPTS RECORD');
			...
			$this->objPHPExcel->getActiveSheet()->getStyle('E4:H4')->getAlignment()->setWrapText(true);
			...
			$this->objPHPExcel->getActiveSheet()->SetCellValue('G4', 'DEPOSITS');
			$this->objPHPExcel->getActiveSheet()->SetCellValue('H4', 'UNDEPOSITED COLLECTION');
		...
		public function renderRow($row)
		{
			...
			$this->objPHPExcel->getActiveSheet()->getStyle('A'.($row+4).':H'.($row+6))->applyFromArray($styleArray);
			...
			$this->objPHPExcel->getActiveSheet()->getStyle('G'.($row+5))->getNumberFormat()->setFormatCode('#,##0.00');
			$this->objPHPExcel->getActiveSheet()->getStyle('H'.($row+5))->getNumberFormat()->setFormatCode('#,##0.00');
		...
		public function renderFooter($row)
		{
			...
			$this->objPHPExcel->getActiveSheet()->setCellValue('G'.($row2+1),'=SUM(G5:G'.($row2).')');
			$this->objPHPExcel->getActiveSheet()->setCellValue('H'.($row2+1),'=SUM(H5:H'.($row2).')');
			...
			$this->objPHPExcel->getActiveSheet()->getStyle('G'.($row2+1))->getNumberFormat()->setFormatCode('#,##0.00');
			$this->objPHPExcel->getActiveSheet()->getStyle('H'.($row2+1))->getNumberFormat()->setFormatCode('#,##0.00');
			...
			$row2 += 3;
			$this->objPHPExcel->getActiveSheet()->mergeCells('A'.$row2.':H'.$row2);
			...
			$row2 += 2;
			$row3 = $row2 + 2;
			$textCert = '     I hereby certify that the foregoing is a correct and complete record of all collections and deposits had by me in my capacity as a Cashier of this center during the period from '.date('M d, Y', strtotime($this->minDate)).' to '.date('M d, Y', strtotime($this->maxDate)).' inclusives, as indicated in the corresponding columns.';
			$this->objPHPExcel->getActiveSheet()->mergeCells('A'.$row2.':H'.$row3);
			...
			$row2 += 5;
			$this->objPHPExcel->getActiveSheet()->mergeCells('F'.$row2.':G'.$row2);
			$this->objPHPExcel->getActiveSheet()->SetCellValue('F'.$row2, 'JALI J. BADIOLA');
		}		
