	...
	class EExcelView extends CGridView
	{
		...
		public function renderRow($row)
		{
			...
			foreach($this->columns as $n=>$column):
				...
				//date edited francis
				//edited by RBG...
				//we need to trigger is_numeric function to make sure numeric date value is processed 
				
				$dateF= explode("-", $value);
				if (is_numeric($dateF[0]) AND is_numeric($dateF[1]) AND is_numeric($dateF[2])){
					$c1=count($dateF);
					
						if($c1==3 && $dateF[0]<9000 && $dateF[1]<13 && $dateF[2]<32){
							//if (is_numeric($dateF[0]) && is_numeric($dateF[1]) && is_numeric($dateF[2])){
				...
				self::$activeSheet->setCellValue( $this->columnName(++$i).($row+2) , htmlspecialchars_decode($value,ENT_QUOTES));
				//special chars we be decoded back to original
			endforeach;
			//self::$activeSheet->setCellValue( $this->columnName(++$i).($row+5) , htmlspecialchars_decode($value));
			
			//self::$activeSheet->setCellValue( $this->columnName($i).($row+6) , $this->totalAmount);
			//self::$activeSheet->setCellValue( "A2" , $this->totalAmount);
		...
		*/
		...
		/*
		public static function html_entities(stack:String):String {
        var arrEnt:Array = [
            ["&amp;", "&"], 
            ["&gt;", ">"], 
            ["&lt;", "<"],
        ...
        var _stack:String = stack;
        for (var i=0; i<arrEnt.length; i++){
            var arr:Array=_stack.split(arrEnt[i][0]);
            _stack = arr.join(arrEnt[i][1]);
        }
