...
* @version 0.1
 */
class LabReportBehavior extends CBehavior
{
	 ...
	 *				return array(
	 *					'eexcelview'=>array(
	 *						'class'=>'ext.eexcelview.CashierReportBehavior',
	 *					),
	 *				);
	 *			}
	 *		Then, to use
	...
	public function toExcel($model=null, $columns=array(), $title=null, $documentDetails=array(), $exportType='Excel2007')
	{
		...
		if(! $this->owner instanceof CController){
			Yii::log('PrintReportBehavior can only be attached to an instance of CControler', 'error', 'system.base.CBehavior');
			return;
		}
		...
		//$this->owner->widget('ext.eexcelview.ExportConsoView', $arg);
		
		
		//if($reportType == 1)
		if($_SESSION['labReport'] == 1)
			$this->owner->widget('ext.eexcelview.ExportConsoView', $arg);
		//if($reportType == 2)
		if($_SESSION['labReport'] == 2)	
			$this->owner->widget('ext.eexcelview.ExportSummaryView', $arg);
