 <?php
	...
	* @version 0.3
	*/	
	class EExcelViewCreateDataEntryFile extends CGridView
	{
		//Document properties
		...
		public function init()
		{
			...
			{			
				...
				$this->objPHPExcel = PHPExcel_IOFactory::load("./protected/data/DataEntryForm.xlsx");
		...
		public function run()
		{
			if($this->grid_mode == 'export')
			{
				/* Disable rendering of header,body and footer
					since we are loading a template file
				*/
				$this->renderHeader();
				$row = $this->renderBody();
				...
				//$this->objPHPExcel->getActiveSheet()->setCellValue("D8", "=VLOOKUP(C8,DropdownLookup!$E$2:$F$4,2,FALSE)");				
						
				$this->objPHPExcel->setActiveSheetIndex(0);
				
				//Lab Id/Type
				$objValidation = $this->objPHPExcel->getActiveSheet()->getCell('C8')->getDataValidation();
				...
				$objValidation->setFormula1('DropdownLookup!$E$2:$E$'.($this->labCount+1));	// Make sure to put the list items between " and "  !!!
				$this->objPHPExcel->getActiveSheet()->setCellValue('D8','=VLOOKUP(C8,DropdownLookup!$E$2:$F$'.($this->labCount+1).',2,FALSE)');
				
				//Payment Id/Type
				$objValidation = $this->objPHPExcel->getActiveSheet()->getCell('E8')->getDataValidation();
				$objValidation->setType( PHPExcel_Cell_DataValidation::TYPE_LIST );
				$objValidation->setErrorStyle( PHPExcel_Cell_DataValidation::STYLE_INFORMATION );
				$objValidation->setAllowBlank(false);
				$objValidation->setShowInputMessage(true);
				...
				$objValidation->setPromptTitle('Pick from list');
				$objValidation->setPrompt('Please pick a value from the drop-down list.');
				$objValidation->setFormula1('DropdownLookup!$A$2:$A$'.($this->paymentTypeCount+1));	// Make sure to put the list items between " and "  !!!
				$this->objPHPExcel->getActiveSheet()->setCellValue('F8', '=VLOOKUP(E8,DropdownLookup!$A$2:$B$'.($this->paymentTypeCount+1).',2,FALSE)');

				//Discount Id/Type
				$objValidation = $this->objPHPExcel->getActiveSheet()->getCell('G8')->getDataValidation();
				$objValidation->setType( PHPExcel_Cell_DataValidation::TYPE_LIST );
				$objValidation->setErrorStyle( PHPExcel_Cell_DataValidation::STYLE_INFORMATION );
				$objValidation->setAllowBlank(false);
				$objValidation->setShowInputMessage(true);
				...
				$objValidation->setPromptTitle('Pick from list');
				$objValidation->setPrompt('Please pick a value from the drop-down list.');
				$objValidation->setFormula1('DropdownLookup!$C$2:$C$'.($this->discountCount+1));	// Make sure to put the list items between " and "  !!!
				$this->objPHPExcel->getActiveSheet()->setCellValue('H8', '=VLOOKUP(G8,DropdownLookup!$C$2:$D$'.($this->discountCount+1).',2,FALSE)');

				//Customer Id/Type
				$objValidation = $this->objPHPExcel->getActiveSheet()->getCell('I8')->getDataValidation();
				$objValidation->setType( PHPExcel_Cell_DataValidation::TYPE_LIST );
				$objValidation->setErrorStyle( PHPExcel_Cell_DataValidation::STYLE_INFORMATION );
				$objValidation->setAllowBlank(false);
				$objValidation->setShowInputMessage(true);
				...
				$objValidation->setPromptTitle('Pick from list');
				$objValidation->setPrompt('Please pick a value from the drop-down list.');
				$objValidation->setFormula1('DropdownLookup!$G$2:$G$'.($this->customerCount+1));	// Make sure to put the list items between " and "  !!!
				$this->objPHPExcel->getActiveSheet()->setCellValue('J8', '=VLOOKUP(I8,DropdownLookup!$G$2:$H$'.($this->customerCount+1).',2,FALSE)');

				//Sample Types/Id
				$objValidation = $this->objPHPExcel->getActiveSheet()->getCell('K8')->getDataValidation();
				$objValidation->setType( PHPExcel_Cell_DataValidation::TYPE_LIST );
				$objValidation->setErrorStyle( PHPExcel_Cell_DataValidation::STYLE_INFORMATION );
				$objValidation->setAllowBlank(false);
				$objValidation->setShowInputMessage(true);
				...
				$objValidation->setPromptTitle('Pick from list');
				$objValidation->setPrompt('Please pick a value from the drop-down list.');
				$objValidation->setFormula1('DropdownLookup!$O$2:$O$'.($this->sampleTypeCount+1));	// Make sure to put the list items between " and "  !!!
				$this->objPHPExcel->getActiveSheet()->setCellValue('L8', '=VLOOKUP(K8,DropdownLookup!$O$2:$P$'.($this->customerCount+1).',2,FALSE)');				
				
				//Test/Params Id/Name
				$objValidation = $this->objPHPExcel->getActiveSheet()->getCell('P8')->getDataValidation();
				$objValidation->setType( PHPExcel_Cell_DataValidation::TYPE_LIST );
				$objValidation->setErrorStyle( PHPExcel_Cell_DataValidation::STYLE_INFORMATION );
				$objValidation->setAllowBlank(false);
				$objValidation->setShowInputMessage(true);
				...
				$objValidation->setPromptTitle('Pick from list');
				$objValidation->setPrompt('Please pick a value from the drop-down list.');
				//$objValidation->setFormula1('DropdownLookup!$I$2:$I$'.($this->testCount+1));	// Make sure to put the list items between " and "  !!!
				$objValidation->setFormula1('OFFSET(DropdownLookup!$K$1,MATCH($L$8,DropdownLookup!$K:$K,0)-1,-2,COUNTIF(DropdownLookup!$K:$K,$L$8),1)');
				$this->objPHPExcel->getActiveSheet()->setCellValue("Q8", '=VLOOKUP(P8,DropdownLookup!$I$2:$N$'.($this->testCount+1).',2,FALSE)');
				$this->objPHPExcel->getActiveSheet()->setCellValue("R8", '=VLOOKUP(P8,DropdownLookup!$I$2:$N$'.($this->testCount+1).',4,FALSE)');
				$this->objPHPExcel->getActiveSheet()->setCellValue("S8", '=VLOOKUP(P8,DropdownLookup!$I$2:$N$'.($this->testCount+1).',5,FALSE)');
				$this->objPHPExcel->getActiveSheet()->setCellValue("T8", '=VLOOKUP(P8,DropdownLookup!$I$2:$N$'.($this->testCount+1).',6,FALSE)');				
