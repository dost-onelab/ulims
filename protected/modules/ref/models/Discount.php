...
class Discount extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('type, rate, status', 'required'),
			array('id, status', 'numerical', 'integerOnly'=>true),
			array('rate', 'numerical'),
			array('type', 'length', 'max'=>50),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, type, rate, status', 'safe', 'on'=>'search'),
		);
	}
	...
	{	
		$discounts = RestController::getAdminData('discounts');
		
		return CHtml::listData($discounts, 'id', 'type');
	}
}
