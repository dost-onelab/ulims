 ...
 */
class Lablocal extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('labName, labCode', 'required'),
			array('status', 'numerical', 'integerOnly'=>true),
			array('labName', 'length', 'max'=>50),
			array('labCode', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, labName, labCode, labCount, nextRequestCode, status', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'initializeCode' => array(self::HAS_MANY, 'Initializecode', 'lab_id'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('labCount',$this->labCount);
		$criteria->compare('nextRequestCode',$this->nextRequestCode,true);
	...
	public static function checkIfInitialized($lab){
		$linkInitialize = Chtml::link('<span class="icon-refresh icon-white"></span> Initialize', '', array(
                ...
                'onClick'=>'js:{initializeCode(); $("#dialogInitializeCode").dialog("open");}',
                ));
		$request=new Request;
		return $lab->initializeCode ? $request->generateRequestRef($lab->id) : $linkInitialize;
	}	
}
