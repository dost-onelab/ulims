...
class Lab extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, rstl', 'required'),
			array('name', 'length', 'max'=>50),
			array('rstl', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, rstl', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			//'labConfig' => array(self::HAS_ONE, 'Configlab', 'lab_id'),
		);
	}
	...
	{
		// @todo Please modify the following rstl to remove attributes that should not be searched.

		$criteria=new CDbCriteria;

		$criteria->compare('id',$this->id);
		$criteria->compare('name',$this->name,true);
		$criteria->compare('rstl',$this->rstl,true);
