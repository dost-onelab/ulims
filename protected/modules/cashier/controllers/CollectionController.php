...
class CollectionController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_GET['id']))
		{
			...
			//set scenario for attribute label
			if($receipt){
				if($receipt->collectionType==1)
					$model->scenario='labCollection';
			}
		}	
		...
		if (Yii::app()->request->isAjaxRequest)
        {
            echo CJSON::encode(array(
                ...
                'div'=>$this->renderPartial('_form', array('model'=>$model,
                	'receiptId'=>$receiptId, 'receipt'=>$receipt), true, true))
            );
            exit;               
        }else{
	...
	public function actionUpdate($id=NULL)
	{
		...
		if (Yii::app()->request->isAjaxRequest)
        {
			echo CJSON::encode(array(
                'status'=>'failure',
                'div'=>$this->renderPartial('_form', array('model'=>$model,'receiptId'=>$receiptId, 'receipt'=>$receipt,
				), true, true)));
	...
	{
	    if(isset($_POST['Collection']['nature'])){
			$requestRefNum=$_POST['Collection']['nature'];
