...
class Check extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('receipt_id, bank, checknumber, checkdate, amount', 'required'),
			array('receipt_id', 'numerical', 'integerOnly'=>true),
			array('amount', 'numerical'),
			array('bank, checknumber', 'length', 'max'=>25),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, receipt_id, bank, checknumber, checkdate, amount', 'safe', 'on'=>'search'),
		);
	}
	...
	public function beforeSave(){
	   ...
	   {
	   		$this->bank = strtoupper($_POST['Check']['bank']);
