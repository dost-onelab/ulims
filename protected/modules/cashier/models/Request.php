...
class Request extends CActiveRecord
{
	...
	public static function listDataUnpaid($model=NULL)
	{
		$criteria=new CDbCriteria;
		$criteria->select='id,requestRefNum,total,labId';
		$criteria->condition='rstl_id = :rstl_id AND t.cancelled=0';
		$criteria->order='id DESC';
		$criteria->params=array(':rstl_id'=>Yii::app()->Controller->getRstlId());
		...
		if($requests){
			foreach ($requests as $request){
				...
				if($balance>0 OR ($model->request_id==$request->id)){ //$model->request_id==$request->id --> needed on update
					...
					'labId'=>Lab::model()->findByPk($request->labId)->labName,
					'balance'=>$balance
					);
				}
	    	}
