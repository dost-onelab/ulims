 ...
 */
class Orderofpaymentlab extends CActiveRecord
{
	...
	public function beforeSave(){
	   if(parent::beforeSave())
	   {
			if($this->isNewRecord){
				...
				$this->date = date('Y-m-d', strtotime($_POST['Orderofpaymentlab']['date']));
				$this->customerName = Customer::model()->findByPk($this->customer_id)->customerName;
				$this->address = Customer::model()->findByPk($this->customer_id)->completeAddress;
                //$this->customerName = 'test 1';
				//$this->address = 'test 2';
		        return true;
			}else{
				$this->date = date('Y-m-d', strtotime($_POST['Orderofpaymentlab']['date']));
