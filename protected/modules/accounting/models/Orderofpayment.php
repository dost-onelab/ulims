...
class Orderofpayment extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('id, transactionNum, collectiontype_id, date, customer_id, customerName, address, amount, purpose', 'safe', 'on'=>'search'),
		);
	}
	...
	public function beforeSave(){
	   if(parent::beforeSave())
	   {
			if($this->isNewRecord){
				...
				$lastOrId = Lastor::model()->find($criteria);
		
				$or = Lastor::model()->findByPk($lastOrId->id);
				
				$lastor = new Lastor;
				$lastor->rstl_id = Yii::app()->Controller->getRstlId();
				$lastor->number = $or->number + 1;
				$lastor->display = $this->addZeros($or->number + 1);
				$lastor->save();*/
				
				//$this->payor = $_POST['receiptPayor'];
				//$this->receiptId = $or->number + 1;
