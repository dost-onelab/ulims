...
class Testcategory extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('categoryName, labId', 'required'),
			array('labId', 'numerical', 'integerOnly'=>true),
			array('categoryName', 'length', 'max'=>200),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, categoryName, labId', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'lab'	=> array(self::BELONGS_TO, 'Lab', 'labId'),
		);
	}
	...
	{
		return CHtml::listData(Testcategory::model()->findAll(), 'id', 'categoryName');
	}
}
