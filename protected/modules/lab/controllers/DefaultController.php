	...
	public function actionIndex()
	{
		...
		if($yearListCategories){
			...
			*/
			...
			$labCriteria=new CDbCriteria;
			$labCriteria->condition='status=:active';
			$labCriteria->params=array(':active'=>1);
			$laboratories=Lab::model()->findAll($labCriteria);
			...
			if($threeYear){
				...
				/* Uncomment code below to get top5 list per year (entire lab) 
				*/
				/*
				...
				$topAnalysis3years[$year]=array('year'=>$year, 
												'topAnalysis'=>new CArrayDataProvider($topAnalysis,array('pagination'=>false)),
												);
				*/
					foreach($laboratories as $lab){
						$labId=$lab->id;
						$topAnalysisLab=Analysis::model()->with(array(
							/*'anals'=>array(
						...
						$topAnalysisByLab[$labId]=array('lab'=>$lab,
														'topAnalysisLab'=>new CArrayDataProvider($topAnalysisLab,array('pagination'=>false))
														);
					}
					$topAnalysis3yearsByLab[$year]=array('year'=>$year,'topAnalysisByLab'=>$topAnalysisByLab);										
				}
			}
			
			//test/analysis by lab
		...
		$this->render('index', array(
				...
				//'topAnalysis3years'=>$topAnalysis3years, //top tests/analysis entire lab
